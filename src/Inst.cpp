...
{
    auto target = m_target->getString();
    auto operand1 = m_operand1->getString();
    auto operand2 = m_operand2->getString();
...
{
    auto target = m_target->getString();
    auto operand = m_operand->getString();
...
{
    auto target = m_target->getString();
    auto source = m_source->getString();
    auto index = m_index->getString();
...
std::shared_ptr<BasicBlock> JumpInst::getTarget() { return m_target; }
